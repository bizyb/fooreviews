...
import services.decorators as decorators
...
class FacetedSearch(object):
	...
	def get_url_tokens(self):
		...
		if len(url_tokens) > 2 and 'o' not in url_tokens:
			# if an o-flag has been removed deliberately, put it back
			# TODO: this is a temporariy fix. what if we have more than three 
			# levels? 
			url_tokens.append('o')
		return url_tokens
	def url_parser(self):
	...
	def qs_builder(self, **kwargs):
		...
		NB: all filter_name and filter_slug values need to be indexed by ES for 
			faceting to work  
		'''
	...
	def shuffle_qs(self, qs):
		...
		We always want the sort parameter to appear at the end of the qs.
		'''
	...
	def _set_flag(self, param_dict, url_prefix, url_tokens):
		...
		taxonomy. Here, appending an 'o' flag lets us know where we are. 
		...
		Why do it this way?
			...
			obviously other ways of doing it, foregoing Elasticsearch entirely.
			However, we have far greater control with this approach.   
		''' 
		if url_tokens[-1] == 'o':
	...
	def get_agg_fields(self, names=False, fields=False, **kwargs):
		...
		Field examples: 
			...
			be searchable by users but also enable us to do aggregations that require exact
			match).
	...
	def filter_query_builder(self, unfiltered=False, **kwargs):
		...
		'''
		filter_list = set_qs_filter(kwargs)
		master_filter = None
		unfiltered = False
		if filter_list:
			try:
	...
	def _set_bool_filtering(self, s, **kwargs):
		'''
		Set user-select drill down filters on ES search instance.
